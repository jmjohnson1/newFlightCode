...
class SITLIMUModel {
 public:
  ...
  // Making a static declaration for the defaults so the compiler is happy
  static const NoiseParams& GetDefaultParams() {
    static const NoiseParams defaults;
    return defaults;
  }

  SITLIMUModel(const NoiseParams& params = GetDefaultParams());

  // Update sensor with true values from physics engine
  void update(double dt, const Eigen::Vector3d& true_accel_body, const Eigen::Vector3d& true_gyro_body,
...
class SITLGPSModel {
 public:
  ...
  void update(const Eigen::Vector3d& ned_position, const Eigen::Vector3d& ned_velocity);
