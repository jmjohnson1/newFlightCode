...
public:
    ...
    void print(unsigned int val, int base = 10) {
        print(static_cast<int>(val), base);
    }
    ...
    void print(double val, int digits = 2) {
        print(static_cast<float>(val), digits);
    }
...
void simulateSerialInput(const std::string& data) {
    serial_usb.addInputData(data);
}

void simulateSerial5Input(const std::string& data) {
    serial5_hw.addInputData(data);
}

// Extended Serial interface (if your code uses these)
extern "C" {
    void serial_print_float_precision(float val, int precision) {
        serial_usb.print(val, precision);
    }
    
    void serial_print_hex(int val) {
        serial_usb.print(val, 16);
    }
    
    void serial_print_binary(int val) {
        serial_usb.print(val, 2);
    }
}
