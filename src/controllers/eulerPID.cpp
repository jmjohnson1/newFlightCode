...
//==================//
/**
 * @brief Create an AngleAttitudeController object with the given PID gains and integral anti-windup limit.
 * @param Kp Reference to a 3-element array containing the P-gain for roll, pitch, and yaw
 * @param Ki Reference to a 3-element array containing the I-gain for roll, pitch, and yaw
 * @param Kd Reference to a 3-element array containing the D-gain for roll, pitch, and yaw
 * @param iLimit The maximum value that the integral term can take. (default = 25.0f)
 */
AngleAttitudeController::AngleAttitudeController(const float (&Kp)[3], const float (&Ki)[3], const float (&Kd)[3],
                                                 float iLimit) {
  for (int i = 0; i < 3; i++) {
    Kp_[i] = Kp[i];
    Ki_[i] = Ki[i];
 ...
 */
void AngleAttitudeController::Update(const Eigen::Vector3f &setpoints, const Eigen::Vector3f &currentAttitude,
 ...
 */
float AngleAttitudeController::AnglePID(float setpoint, float measuredAngle, float gyroRate, float dt, bool noIntegral,
                                        AxisToControl axis) {
 ...
 * @returns The pid output
 */
float AngleAttitudeController::RatePID(float setpoint, float measuredRate, float dt, bool noIntegral,
                                       AxisToControl axis) {
 ...
 * @param noIntegral  Condition on whether to use the integral term
 */
void PositionController::Update(const Eigen::Vector3d &posSetpoints, const Eigen::Vector3f &velocitySetpoints,
                                const Eigen::Vector3d &currentPosition, const Eigen::Vector3f &currentVelocity,
                                const Eigen::Vector3f &currentAttitude, float dt, bool noIntegral) {
  ...
  desAcc_b3 = -desiredThrust_ / mass_;  // It's easier to constrain the thrust. maybe
  ...
  if (abs(desiredThrust_ - minThrust_) > EPSILON) {
    ...
    sinPitch = (Cy * desAcc_ned[0] + Sy * desAcc_ned[1]) / desAcc_b3 / sqrt(1 - sinRoll * sinRoll);
